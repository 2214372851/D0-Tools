...
class SelectDialog:
    ...
    def __init__(self, canvas):
        ...
        self.language = self.translate.getLanguage
    ...
    def showSelect(self):
        ...
        """
        self.state = False
        option_window = QtWidgets.QDialog(self.canvas)
        option_window.setModal(True)
        option_window.setMinimumSize(QtCore.QSize(0, 0))
        option_window.setWindowFlags(QtCore.Qt.WindowType.FramelessWindowHint | QtCore.Qt.WindowType.Dialog)
        ...
        save_button.setText(self.translate.get_tr('确定'))
        ...
        cancel_button.setText(self.translate.get_tr('取消'))
    ...
    def showPosition(option_window):
        ...
        if (
                QtGui.QCursor.pos().y() + option_window.height() + 100) < QtGui.QGuiApplication.primaryScreen().geometry().height():
    ...
    def showOptions(self, vertical_layout: QtWidgets.QVBoxLayout, dialog: QtWidgets.QDialog):
        ...
        for option_name, option in self.options.items():
            ...
            if option_type == 'select':
                ...
                comboBox.addItems(option['options'])
                if value:
                    comboBox.setCurrentIndex(option['options'].index(value))
                else:
                ...
                for index, item in enumerate(option['options']):
