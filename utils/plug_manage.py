...
class PlugManage(AutoTree.Tree):
    ...
    def startModule(self, module_name):
        ...
        if module.Module.isConfig:
            if not self.module.sidebar:
                ...
                widgets = self.initEntirety()
                self.main_ui.addToolListButton(widgets)
            self.module.setModuleEntirety(self.entirety)
            last_filename = self.main_ui.project_settings.value('LASTFILENAME')
            if last_filename: self.openFile(Path(str(last_filename)))
            self.main_ui.widget_toast.showSucceed(self.main_ui.getTr('打开成功'))

    def initEntirety(self):
        """
        初始整体选项
        :return:
        """
        if not self.module: return
        language = self.main_ui.tools_tr.getLanguage
        widget_list = []
        for option in self.module.plugOption.image_options:
    ...
    def initTreeItem(self):
        ...
        self.main_ui.widget_toast.showSucceed(self.main_ui.getTr('读取插件完成'))
